...
public class UpdateCartController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            int cartQuantity = Integer.parseInt(request.getParameter("txtCartQuantity"));
            int quantity = Integer.parseInt(request.getParameter("txtQuantity"));
            boolean valid = true;
            if (cartQuantity > quantity) {
                valid = false;
                request.setAttribute("WARN", "In Cart are exceed In Stock.");
            }
            if (valid) {
                HttpSession session = request.getSession();
                ShoppingCartDTO cart = (ShoppingCartDTO) session.getAttribute("CART");
                cart.updateCart(id, cartQuantity);
